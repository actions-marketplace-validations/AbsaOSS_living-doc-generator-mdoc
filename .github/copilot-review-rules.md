...
- Domain-specific high-risk areas
  - Input file handling: validate `source` path, encoding, and missing/invalid JSON.
  - Template rendering: ensure templates/ changes don’t break MDOC output structure.
  - Output layout: keep generated file naming and directory structure stable (especially under `output-path`).
  - Report page: when `report-page` is enabled, ensure error summaries remain stable and safe to render.
  - Logging: avoid leaking file contents or environment values in logs; keep lazy % formatting.
