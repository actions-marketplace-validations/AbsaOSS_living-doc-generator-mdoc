...
- High-risk areas
  - Input file handling: `source` path validation and missing/invalid JSON.
  - Template rendering: templates/ changes can silently break output structure.
  - Output layout: file naming and directory structure under `output-path`.
  - Logging: avoid leaking file contents or environment values.
