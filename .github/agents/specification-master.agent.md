...
- Spec locations
  - Prefer README.md (user contract) and DEVELOPER.md (dev contract) as sources of truth.
  - Prefer action.yml as the canonical inputs/outputs contract for the GitHub Action.
- Contract-sensitive outputs
  - Generated MDOC files/layout; report page content (when enabled); action output output-path; exit codes.
- High-risk areas
  - Input schema drift: raw input JSON shape changes can break generation.
  - Template/output drift: templates/ changes can break MDOC viewer compatibility.
  - Output layout stability: file naming/paths under `output-path`.
