...
Repo additions
- Project name: living-doc-generator-mdoc
- Entry points: action.yml, main.py
- Core packages: living_doc_generator/; utils/
- Input layer: ActionInputs in action_inputs.py (reads INPUT_* env vars via composite action)
- Logging wiring: living_doc_utilities.logging_config.setup_logging
- Action outputs wiring: living_doc_utilities.github.utils.set_action_output
- Contract-sensitive outputs: generated MDOC files/layout; report page content (when enabled); action output output-path; exit codes
- Commands:
  - python3 -m venv .venv
